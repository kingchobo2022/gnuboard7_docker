 ...
 *   오버라이드로 모바일에서만 펼친 목록이 된다(트리거 숨김 + 패널 static).
 *
 * 회귀 차단:
...
describe('헤더 통화 슬롯(이커머스 주입) + 언어 버튼(코어 내장)', () => {
  ...
  it('드로어 언어 칩은 flex-wrap 컨테이너 + Button 에 iteration (가로 나열)', () => {
    ...
    // [0] = 라벨, [1] = 칩 컨테이너
    const chipRow = langSection.children[1];
    expect(chipRow.props.className).toContain('flex');
    expect(chipRow.props.className).toContain('flex-wrap');
...
describe('이커머스 헤더 통화 주입 확장 (slot=header_currency)', () => {
   ...
   */
  it('유저 확장 트리거 버튼은 portable(모바일/태블릿)에서 숨겨진다', () => {
    const node = userExt.injections[0].components[0];
  ...
  it('유저 확장 패널은 portable 에서 항상 펼쳐진 정적 목록이다 (absolute/z-50 해제)', () => {
    const node = userExt.injections[0].components[0];
