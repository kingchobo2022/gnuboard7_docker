/**
 * @file admin-plugin-update.test.tsx
 * @description 플러그인 업데이트 UI 레이아웃 테스트
 *
 * 테스트 대상:
 * - admin_plugin_list.json: updating 배지, 업데이트 확인 버튼, 행별 업데이트 버튼
 * - partials/admin_plugin_list/_modal_update.json: 업데이트 확인 모달
 *
 * 검증 항목:
 * - updating 상태 배지 렌더링
...
const translations = {
  admin: {
    plugins: {
      ...
      check_updates_no_updates: '모든 플러그인이 최신 상태입니다.',
      status: {
        active: '활성화',
        inactive: '비활성화',
        not_installed: '미설치',
      ...
      modals: {
        update_title: '플러그인 업데이트 확인',
        update_confirm: '{{name}} 플러그인을 업데이트하시겠습니까?',
...
describe('플러그인 업데이트 - check-updates auth_required 검증', () => {
  it('check-updates apiCall에 auth_required: true가 설정되어 있다', async () => {
    const fs = await import('fs');
    const path = await import('path');
    const layoutPath = path.resolve(__dirname, '../../layouts/admin_plugin_list.json');
...
describe('플러그인 업데이트 - 상태 배지', () => {
  ...
  function createBadgeLayout(row: Record<string, any>) {
    return {
      version: '1.0.0',
      layout_name: 'plugin_update_badge_test',
      initGlobal: { row },
      components: [
        {
          id: 'main-content',
          ...
          children: [
            {
              ...
              text: '$t:admin.plugins.status.updating',
            },
            {
              type: 'basic',
              name: 'Span',
              ...
              text: '$t:admin.plugins.update_available',
            },
            {
              type: 'basic',
              name: 'Span',
              ...
              text: "$t:admin.plugins.version_available|version={{_global.row.latest_version ?? ''}}",
            },
          ],
        },
      ],
...
describe('플러그인 업데이트 - 업데이트 확인 버튼', () => {
  let testUtils: ReturnType<typeof createLayoutTest>;
  let registry: ComponentRegistry;

  const checkUpdatesButtonLayout = {
    version: '1.0.0',
    layout_name: 'plugin_check_updates_button_test',
    initGlobal: {
        ...
        children: [
          {
            ...
            children: [
              ...
              },
              {
                type: 'basic',
                name: 'Span',
                text: '$t:admin.plugins.actions.check_updates',
              },
            ],
          },
        ],
...
describe('플러그인 업데이트 - 행별 업데이트 버튼', () => {
  ...
  function createRowButtonLayout(row: Record<string, any>) {
    return {
      version: '1.0.0',
      layout_name: 'plugin_row_update_button_test',
      initGlobal: { row },
      components: [
        {
          id: 'main-content',
          ...
          children: [
            {
              ...
              children: [
                ...
                },
                {
                  type: 'basic',
                  name: 'Span',
                  text: '$t:admin.plugins.actions.update',
                },
              ],
            },
          ],
...
describe('플러그인 업데이트 - 업데이트 모달', () => {
  let testUtils: ReturnType<typeof createLayoutTest>;
  let registry: ComponentRegistry;

  const updateModalLayout = {
    version: '1.0.0',
    layout_name: 'plugin_update_modal_test',
    initGlobal: {
      selectedPlugin: {
        ...
        github_changelog_url: 'https://github.com/gnuboard/sirsoft-payment/releases',
      },
      isPluginUpdating: false,
    },
    modals: [
      {
        id: 'plugin_update_modal',
        type: 'composite',
        name: 'Modal',
        props: {
          title: '$t:admin.plugins.modals.update_title',
          size: 'sm',
        },
        children: [
          {
            ...
            children: [
              {
                ...
                text: "$t:admin.plugins.modals.update_confirm|name={{_global.selectedPlugin.name ?? ''}}",
              },
              {
                type: 'basic',
                name: 'Div',
                    ...
                    children: [
                      {
                        ...
                        text: '$t:admin.plugins.modals.field_version',
                      },
                      {
                        type: 'basic',
                        name: 'Span',
                        props: { className: 'font-medium text-gray-900 dark:text-white' },
                        text: "$t:admin.plugins.modals.update_version_info|from={{_global.selectedPlugin.version ?? ''}}|to={{_global.selectedPlugin.latest_version ?? ''}}",
                      },
                    ],
                  },
                  {
                    ...
                    children: [
                      {
                        ...
                        name: 'Span',
                        props: { className: 'text-gray-500' },
                        text: '$t:admin.plugins.modals.field_vendor',
                      },
                      {
                        type: 'basic',
                        name: 'Span',
                        props: { className: 'text-gray-700' },
                        text: "{{_global.selectedPlugin.update_source === 'github' ? '$t:admin.plugins.modals.update_source_github' : '$t:admin.plugins.modals.update_source_bundled'}}",
                      },
                    ],
                  },
                ],
              },
              {
                type: 'basic',
                name: 'Div',
                if: '{{_global.selectedPlugin.github_changelog_url}}',
                ...
                children: [
                  {
                    ...
                    name: 'A',
                    props: {
                      href: '{{_global.selectedPlugin.github_changelog_url}}',
                      target: '_blank',
                    ...
                    children: [
                      ...
                      },
                      {
                        type: 'basic',
                        name: 'Span',
                        text: '$t:admin.plugins.modals.view_changelog',
                      },
                    ],
                  },
                ],
            ...
            children: [
              {
                ...
                props: {
                  ...
                  disabled: '{{_global.isPluginUpdating}}',
                ...
                children: [
                  {
                    type: 'basic',
                    name: 'Icon',
                    if: '{{_global.isPluginUpdating}}',
                    props: { name: 'spinner', className: 'w-4 h-4 animate-spin' },
                  },
                  {
                    type: 'basic',
                    name: 'Span',
                    text: "{{_global.isPluginUpdating ? '$t:admin.plugins.updating' : '$t:admin.plugins.actions.update'}}",
                  },
                ],
              },
            ],
  ...
  describe('레이아웃 구조 검증', () => {
    it('레이아웃 정보가 올바르게 로드된다', () => {
      ...
      expect(info.name).toBe('plugin_update_modal_test');
    ...
    it('초기 전역 상태가 올바르게 설정된다', () => {
      ...
      expect(state._global.isPluginUpdating).toBe(false);
    });
  ...
  describe('모달 열기/닫기', () => {
    it('업데이트 모달을 열고 닫을 수 있다', async () => {
      ...
      testUtils.openModal('plugin_update_modal');
      expect(testUtils.getModalStack()).toContain('plugin_update_modal');

      testUtils.closeModal();
      expect(testUtils.getModalStack()).not.toContain('plugin_update_modal');
    });
  });
  ...
  describe('모달 콘텐츠 구조 검증', () => {
    it('모달 타이틀이 업데이트 확인으로 설정되어 있다', () => {
      const modal = updateModalLayout.modals[0];
      expect(modal.props.title).toBe('$t:admin.plugins.modals.update_title');
    });

    it('모달에 업데이트 확인 메시지가 포함되어 있다', () => {
      const modal = updateModalLayout.modals[0];
      const confirmP = modal.children[0].children[0];
      expect(confirmP.text).toContain('$t:admin.plugins.modals.update_confirm');
      expect(confirmP.text).toContain('_global.selectedPlugin.name');
    });
    ...
    it('모달에 버전 정보가 포함되어 있다 (from → to)', () => {
      ...
      expect(versionLabel.text).toBe('$t:admin.plugins.modals.field_version');
      expect(versionValue.text).toContain('$t:admin.plugins.modals.update_version_info');
      expect(versionValue.text).toContain('_global.selectedPlugin.version');
      expect(versionValue.text).toContain('_global.selectedPlugin.latest_version');
    });
    ...
    it('모달에 update_source 표시가 포함되어 있다', () => {
      ...
      expect(vendorLabel.text).toBe('$t:admin.plugins.modals.field_vendor');
    ...
    it('모달에 changelog 링크가 조건부로 포함되어 있다', () => {
      ...
      expect(changelogDiv.if).toBe('{{_global.selectedPlugin.github_changelog_url}}');

      const linkA = changelogDiv.children[0];
      expect(linkA.name).toBe('A');
      expect(linkA.props.target).toBe('_blank');
      expect(linkA.props.href).toContain('_global.selectedPlugin.github_changelog_url');
    });

    it('changelog URL이 없으면 조건에 의해 렌더링되지 않는다', () => {
      const noChangelogState = { ...updateModalLayout.initGlobal.selectedPlugin, github_changelog_url: null };
  ...
  describe('버튼 구조 및 상태 검증', () => {
    it('취소/확인 버튼이 모달에 정의되어 있다', () => {
      ...
      expect(cancelBtn.props.disabled).toBe('{{_global.isPluginUpdating}}');
      expect(cancelBtn.children[0].text).toBe('$t:common.cancel');

      expect(confirmBtn.name).toBe('Button');
      expect(confirmBtn.props.disabled).toBe('{{_global.isPluginUpdating}}');
    });
    ...
    it('확인 버튼에 스피너 조건부 렌더링이 설정되어 있다', () => {
      ...
      expect(spinnerIcon.if).toBe('{{_global.isPluginUpdating}}');
      expect(spinnerIcon.props.name).toBe('spinner');

      const textSpan = confirmBtn.children[1];
      expect(textSpan.text).toContain('_global.isPluginUpdating');
      expect(textSpan.text).toContain('$t:admin.plugins.updating');
      expect(textSpan.text).toContain('$t:admin.plugins.actions.update');
    });

    it('isPluginUpdating 상태가 올바르게 변경될 수 있다', async () => {
      testUtils = createLayoutTest(updateModalLayout, {
        translations,
        locale: 'ko',
        componentRegistry: registry,
      ...
      testUtils.setState('isPluginUpdating', true, 'global');
      state = testUtils.getState();
      expect(state._global.isPluginUpdating).toBe(true);

      testUtils.setState('isPluginUpdating', false, 'global');
