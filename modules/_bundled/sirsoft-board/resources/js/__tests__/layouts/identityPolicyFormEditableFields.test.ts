/**
 * 게시판 본인인증 정책 편집 모달 — 선언형 정책의 편집 가능/잠금 필드 회귀 테스트.
 *
...
const modalPartial = require('../../../layouts/admin/partials/admin_board_settings/_modal_identity_policy_form.json');
...
describe('게시판 본인인증 정책 모달 — 편집 가능/잠금 필드', () => {
    const lockedFields = [
    ...
    it('purpose Select 옵션은 전체 purpose 목록 매핑 (필터 없이 inicis.adult_verification 포함 가능)', () => {
        const purpose = findFieldControl(modalPartial, '$t:admin.identity.policies.form.purpose');
        expect(purpose!.props?.options).toContain('boardIdentityPurposes');
    ...
    it('우선순위(priority) 입력칸이 존재하고 form.priority 에 바인딩된다', () => {
        ...
        // 선언형 정책도 우선순위는 편집 가능 (disabled 미부여)
