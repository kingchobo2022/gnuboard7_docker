...
describe('이커머스 알림 템플릿 모달 회귀 (옵션 K)', () => {
  ...
  it('role 드롭다운 options 가 현재 선택값을 시드', () => {
    const modalStr = JSON.stringify(notifModal);
    expect(modalStr).not.toContain('"options":"{{_global.ecommerce_notification_template_form_modal?.roleSearchResults ?? []}}"');
  ...
  it('specific_users 드롭다운 options 가 현재 선택값을 시드', () => {
    const modalStr = JSON.stringify(notifModal);
    expect(modalStr).not.toContain('"options":"{{_global.ecommerce_notification_template_form_modal?.userSearchResults ?? []}}"');
